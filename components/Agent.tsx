...
const handleCall = async () => {
    ...
    try {
      ...
      setCallStatus(CallStatus.FINISHED); // Fallback
    }
  };

  const handleDisconnect = async () => {
    if (callStatus !== CallStatus.FINISHED) {
      await vapi.stop(); // Actually terminate the call
     ...
     {messages.length>0 && (
      <div className="transcript-border">
        <div className="transcript">
          <p key={latestMessages} className={cn('transition-opacity duration-600 opacity-0', 'animate-fadeIn opacity-100')}>
            {latestMessages  }
          </p>
        </div>
      </div>
